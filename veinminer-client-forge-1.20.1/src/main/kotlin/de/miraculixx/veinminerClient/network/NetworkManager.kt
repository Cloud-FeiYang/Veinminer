...
object NetworkManager : ClientCallbacks {
    ...
    override fun onConfiguration(packet: ServerConfiguration) {
        ...
        if (packet.outdated) {
            Minecraft.getInstance().toasts.addToast(
                SystemToast(SystemToast.SystemToastIds.PERIODIC_NOTIFICATION, Component.literal("Veinminer Outdated"), Component.literal("Please update Veinminer"))
            )
        }
    ...
    private fun parseId(raw: String): ResourceLocation? = try {
        ResourceLocation.tryParse(raw) ?: ResourceLocation(raw)
    } catch (_: Exception) {
        null
    }
