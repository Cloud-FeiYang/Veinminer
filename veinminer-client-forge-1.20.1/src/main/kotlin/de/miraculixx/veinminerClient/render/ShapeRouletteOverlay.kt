...
object ShapeRouletteOverlay {
    ...
    private fun drawPanel(g: GuiGraphics, x1: Int, y1: Int, x2: Int, y2: Int, alpha: Int) {
        ...
        g.fill(x1, y1, x2, y1 + 1, mixAlpha(alpha, 0x60, 0xFFFFFF))
        g.fill(x1, y2 - 1, x2, y2, mixAlpha(alpha, 0x60, 0x000000))
        g.fill(x1, y1, x1 + 1, y2, mixAlpha(alpha, 0x40, 0xFFFFFF))
        g.fill(x2 - 1, y1, x2, y2, mixAlpha(alpha, 0x40, 0x000000))
    }
...
object ForgeShapeRouletteRenderer : IGuiOverlay {
    override fun render(gui: ForgeGui, graphics: GuiGraphics, partialTick: Float, width: Int, height: Int) {
        ShapeRouletteOverlay.render(graphics, partialTick)
    }
}
