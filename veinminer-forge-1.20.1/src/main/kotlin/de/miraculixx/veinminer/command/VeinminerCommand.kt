...
object VeinminerCommand {
    ...
    private fun usage(syntax: String) = error("Correct Syntax: ") + command(syntax).setStyle(Style.EMPTY.withColor(TextColor.fromRgb(cBase)))
    ...
    private fun cmp(text: String, color: Int = cBase, bold: Boolean = false, italic: Boolean = false) =
        Component.literal(text).setStyle(Style.EMPTY.withColor(TextColor.fromRgb(color)).withBold(bold).withItalic(italic))
